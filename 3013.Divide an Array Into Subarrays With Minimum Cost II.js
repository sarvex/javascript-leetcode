...
class AvlTree {
  ...
  add(...items) {
    for (const element of items) {
      this._root = this._addFromNode(element, this._root)
    }
  }
  //#endregion
  ...
  _forLeftToRightNode(fn, node, startOrder) {
    ...
    this._forLeftToRightNode(fn, node.right, startOrder + (node.left?.size ?? 0) + 1)
  }
  /**
   * @param {(item: TItem, order: number) => any} fn
   */
  ...
  _forRightToLeftNode(fn, node, startOrder) {
    ...
    this._forRightToLeftNode(fn, node.left, startOrder - (node.right?.size ?? 0) - 1)
  }
  ...
  remove(...items) {
    for (const element of items) {
      this._root = this._removeFromNode(element, this._root)
    }
  }
...
const minimumCost = (nums, k, dist) => {
  ...
  // For this problem, we need k-1 partition points
  const needCount = k - 1;

  // Use AVL tree to efficiently maintain a sorted window
  ...
  // Initialize the window with the first potential elements
  for (let i = 1; i <= Math.min(n - 1, dist + 1); i++) {
    tree.add(nums[i]);
  }

  // Get the sum of k-1 smallest elements
  let minSum = 0;
  for (let i = 0; i < needCount && i < tree.size; i++) {
    minSum += tree.getItemAt(i);
  }
  ...
  for (let i = dist + 2; i < n; i++) {
    ...
    const outElement = nums[i - dist - 1];
    tree.remove(outElement);

    // Recalculate sum efficiently
    let currentSum = 0;
    for (let j = 0; j < needCount && j < tree.size; j++) {
      currentSum += tree.getItemAt(j);
    }
    
    minSum = Math.min(minSum, currentSum);
  }

  return minSum + firstElement;
}
